...
export class TechView {
  ...
  // Called externally when the tech panel opens/closes so canvases get correct dims.
  resize() {
  ...
  _updateStatus(frame) {
    ...
    _set("val-hue", hue + "°");
    document.getElementById("hue-swatch").style.background = `hsl(${hue},80%,55%)`;
  ...
  _updateDescriptors(frame) {
    ...
    _setBar("flux",             this._smoothFlux,            2);
  ...
  _drawFFT(bins) {
    ...
    // Per-bin temporal smoothing: fast attack, slow decay.
    const ATTACK = 0.8;
    const DECAY  = 0.15;
    ...
    for (let i = 0; i < bins.length; i++) {
      const v = bins[i];
      this._smoothBins[i] = v > this._smoothBins[i]
        ? this._smoothBins[i] + (v - this._smoothBins[i]) * ATTACK
        : this._smoothBins[i] + (v - this._smoothBins[i]) * DECAY;
    }

    const xs = Float32Array.from({ length: bins.length }, (_, i) => ((i + 0.5) / bins.length) * W);
    const ys = Float32Array.from(this._smoothBins, (v) => CHART_H - v * CHART_H);
    ...
    for (const [hz, label] of TechView.FREQ_TICKS) {
      const x = Math.floor(TechView._freqToX(hz) * W);
      ctx.strokeStyle = "#333";
      ctx.lineWidth = 1;
      ctx.beginPath();
  ...
  static _freqToX(hz) {
    ...
    return (Math.log10(Math.max(20, Math.min(20000, hz))) - LOG_MIN) / (LOG_MAX - LOG_MIN);
  }
}
