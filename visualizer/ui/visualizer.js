export class Visualizer {
  ...
  draw() {
    ...
    // Hue tracks spectral centroid, low-passed so it doesn't flicker
  ...
  drawRadialSpectrum() {
    const a        = this.analyzer;
    const ctx      = this.ctx;
    // a.fft is already log-spaced — use linear indexing to preserve that resolution
    const bins      = a.fft.length;   // 64 log-spaced bins from Python
    const usable    = Math.floor(bins * 0.88); // drop top ~8 kHz+ bins
    const barCount  = 200;
    const baseR     = Math.min(this.w, this.h) * 0.17;
    const maxBarH   = Math.min(this.w, this.h) * 0.27;
    ...
    for (let i = 0; i < barCount; i++) {
      const t   = i / barCount;
      // Linear index — bins are already log-spaced by Python, no extra remapping needed
      const idx = Math.min(usable - 1, Math.floor(t * usable));
      const v   = a.fft[idx]; // 0-1 directly
      const len = Math.pow(v, 1.8) * maxBarH;
      if (len < 1) continue;
      ...
      ctx.strokeStyle = `hsla(${(this.hue + t * 120) % 360},85%,62%,0.92)`;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(cos * baseR,       sin * baseR);
      ctx.lineTo(cos * (baseR + len), sin * (baseR + len));
      ctx.stroke();
    }
  ...
  drawCorePulse() {
    ...
    const pulseR = baseR * (0.6 + bass * 0.7);
    ...
    ctx.beginPath();
    ctx.arc(0, 0, baseR * 0.4, 0, Math.PI * 2);
    ctx.stroke();
  ...
  drawWaveformRing() {
    const a      = this.analyzer;
    const ctx    = this.ctx;
    const data   = a.waveform; // 0-1, centred at 0.5
    const baseR  = Math.min(this.w, this.h) * 0.17;
    const amp    = baseR * 0.18;
    ...
    for (let i = 0; i < data.length; i += step) {
      ...
      const r     = baseR * 0.78 + (data[i] - 0.5) * 2 * amp; // 0-1 → -1..1 → radius offset
      if (first) { ctx.moveTo(Math.cos(angle) * r, Math.sin(angle) * r); first = false; }
